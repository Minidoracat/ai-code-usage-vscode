...
extension 只會讀取你設定或透過本機來源偵測核准的 usage paths。此 repository 的公開 screenshots 與 examples 必須使用 synthetic fixture data，不能使用真實 Claude Code 或 Codex 歷史資料。
...
Marketplace screenshots 必須只使用 synthetic fixture data 產生。不要擷取真實 `.claude` 或 `.codex` 使用資料作為公開文件。
